...
![](images/lec-9/asthma.png){width="200" fig-alt="A table with the title Primary Endpoint Annual asthma exacerbation rate over 48 weeks that states the number of patients analyzed is 267, and the rate estimate is 1.33 with a 95% confidence interval of 1.12 to 1.58"}
...
-   Recall from earlier: Given a random variable $X$ with mean $\mu$ and standard deviation $\sigma$, the CLT tells us that
...
- In words: `qnorm(p, mean, sd)` is finding the quantile of the normal distribution corresponding to $(p \times 100)\%$ of the area under the curve falling to the left of this value, assuming a normal distribution with `mean` and `sd`. 
...
- `summarize()` produces a data table with as many rows as there are "groups", and a column for each component of the group and each summary statistic.
