...
- Please reach out to me if you need assistance outside of office hours, I am happy to schedule a time to meet!
...
On the provided paper, identify the target population and what represents an individual case for each of the questions above.
:::

## Quarto recap

Yesterday we discussed using Quarto for producing reports. A couple of notes on Quarto:

- To have Quarto evaluate your code, you use formatting like this:
...
- In Homework 1, the code chunk for Exercise 4 also included the line `#| lab: ex-4`. When a line begins with `#|`, that signals to Quarto that it contains information about the chunk itself, not the code you are evaluating.

- We will talk more about these different options in lab on Friday!
...
#| fig.alt: "Bar chart with four bars, x-axis is region, y-axis is count. The title is Number of US States by Census Region; the Midwest bar has value 12, the Northeast bar has value 10, the South bar has value 15, and the West bar has value 13."
...
```{r fig.height = 8, fig.width = 7, fig.asp=1}
#| fig.alt: "A graph with title Adult Obesity Percentage by State HDI, with x-axis titled HDI and y-axis titled percent adult obesity. Over Bottom ten on the x axis there is a white box going from about 33 to 36 on the y axis, with a black line through it around 35 and a black dot below it at 28. Over Middle there is a white box going from about 27.5 to 32 on the y axis with a black line through it around 30. Over Top ten there is a white box going from about 24 to 28 on the y axis with a black line through it around 26."
...
#| fig.alt: "A histogram with 9 bars titled Distribution of adult obesity percentage by state, with x-axis titled adult obesity percentage and y-axis titled count. The highest bar goes from 31%-33% and has count 14, and the lowest bar goes from 21%-23% and has count 1."
...
#| fig.alt: "A line plot with title Obesity by State, with x-axis labeled state and y-axis labeled adult obesity percentage. Above Alabama, there is a black dot around 26%, then a line going down from that dot to another above Alaska at around 31.5%, then a line going down from that dot to another above Arizona at around 29%, and then a line going up from that dot to another above Arkansas at around 36%."
...
#| fig-alt: "A three-panel scatterplot titled Adequate aerobic activity associated with lower obesity by State HDI, with left panel for bottom ten HDI, center panel for Middle 30 HDI, and right panel for top 10 HDI. The x-axis is labeled adequate aerobic activity percentage and the y-axis is labeled obesity. The points in all three panels show a downward trend in obesity as the percentage of adequate aerobic activity increases."
