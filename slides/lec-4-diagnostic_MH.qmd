...
- Please take advantage of office hours if you need help

- **HW 3** due tonight by 11:59 pm
...
![](images/lec-4/fullprobspace.png){fig-alt="A large white rectangle with two circles inside it, a slightly off-center blue one and a red onethat overlaps the blue circle at its top right corner."}

## Probability of $A$

If we're interested in $P(A)$, this is the information we have:

![](images/lec-4/justblue.png){fig-alt="A white rectangle with an off-center blue circle towards the middle."}

## Probability of $A\vert B$

But if we know that Event $B$ occurred, our sample space "shrinks" down to the red circle:

![](images/lec-4/redhappened.png){fig-alt="A light red circle, with a blue oval over the bottom left part."}
...
- When we have a full table with true status and test results, we can calculate all five of these values.
...
| Positive Test |   54          |  60        | 114            |
| Negative Test |   6           |  180       | 186            |
| **Total**     | <u>**60**</u> |  240       | <u>**300**</u> |

$$\frac{60}{300} = 20\%$$

## Example {.smaller}

What is the sensitivity of this test ($P(Test+\vert Sick)$)?
...
|               | Has Illness   | No Illness | Total |
|---------------|:-------------:|:----------:|:-----:|
| Positive Test | <u>**54**</u> |  60        | 114   |
| Negative Test |   6           |  180       | 186   |
| **Total**     | <u>**60**</u> |  240       | 300   |

$$\frac{54}{60} = 90\%$$

## Example {.smaller}

What is the specificity of this test ($P(Test-\vert Healthy)$)?
...
|---------------|:-----------:|:---------------:|:-----:|
| Positive Test | 54          |  60             | 114   |
| Negative Test | 6           |  <u>**180**</u> | 186   |
| **Total**     | 60          |  <u>**240**</u> | 300   |

$$\frac{180}{240} = 75\%$$

## Example {.smaller}

What is the positive predictive value of this test ($P(Sick\vert Test +)$)?
...
|               | Has Illness   | No Illness | Total          |
|---------------|:-------------:|:----------:|:--------------:|
| Positive Test | <u>**54**</u> |  60        | <u>**114**</u> |
| Negative Test | 6             |  180       | 186            |
| **Total**     | 60            |  240       | 300            |

$$\frac{54}{114} \approx 47\%$$

## Example {.smaller}

What is the negative predictive value of this test ($P(Healthy\vert Test -)$)?
...
|---------------|:-----------:|:---------------:|:---------------:|
| Positive Test | 54          |  60             | 114             |
| Negative Test | 6           |  <u>**180**</u> | <u>**186**</u> |
| **Total**     | 60          |  240            | 300             |

$$\frac{180}{186} \approx 97\%$$
...
## Rapid self-administered HIV tests {.smaller}
...
## The Law of Total Probability {.smaller}
...
- Now we know all of these probabilities (since $P(A^C) = 1 - P(A)$)!
...
$$
\begin{aligned}
P(HIV + | Test +) &=\frac{(.993)(.25)}{(.993)(.25) + (1-.998)(1-.25)}\\
&= 0.99 = 99\%
\end{aligned}
$$
...
-   This is something to look out for! Specificity is important to consider.

## Takeaway

- In the US, small prevalence + modest false positive rate (FPR) is causing the PPV to be much lower than we'd want. 
...
- With a modest FPR, a higher prevalence can help our PPV be higher, yielding a **more useful diagnostic test** for that setting. With lower prevalence, you need to be careful with the conclusions made by a diagnostic test as it could be misleading. 

## Steps to solve using Bayes' Rule

1. Define two events of interest as $A$ and $B$. 

2. Identify what probabilities you already know.

3. Write the probability of interest in terms of Bayes' Rule.

4. Expand denominator in terms of law of total probability.

5. Plug in probabilities from step 2 and solve. 

## Practice calculating NPV with Bayes' Rule 

- Suppose we want to find the NPV of a home COVID-19 test in a population with a 5% prevalence.

- These tests have sensitivity = 85% and specificity = 99%.

- What is the probability of someone being COVID-free, having gotten a negative test result?

## Step 1. Define the events of interest

## Step 1. Define the events of interest

Let $A$ be having COVID and $B$ be a positive test. 

This means that $A^C$ is being COVID-free and $B^C$ is a negative test.
...
$P(B \vert A) = 0.85$ ... therefore $P(B^C \vert A) = 1 - 0.85 = 0.15$

$P(B^C \vert A^C) = 0.99$

$P(A) = 0.05$ ... therefore $P(A^C) = 1 - 0.05 = 0.95$
...
$$
\begin{aligned}
P(A^C \vert B^C) &= \frac{P(A^C \cap B^C)}{P(B^C)}\\
&= \frac{P(B^C \vert A^C)P(A^C)}{P(B^C)}
\end{aligned}
$$
...
$$
\begin{aligned}
P(A^C \vert B^C) &= \frac{P(B^C \vert A^C)P(A^C)}{P(B^C)}\\
&= \frac{P(B^C \vert A^C)P(A^C)}{P(B^C \vert A^C)P(A^C)+P(B^C\vert A)P(A)}\\
\end{aligned}
$$
...
$$
\begin{aligned}
P(A^C \vert B^C) &= \frac{P(B^C \vert A^C)P(A^C)}{P(B^C \vert A^C)P(A^C)+P(B^C\vert A)P(A)}\\
&= \frac{0.99 \times 0.95}{0.99 \times 0.95 + 0.15 \times 0.05}\\
&= 0.992 = 99.2\%
\end{aligned}
$$
...
Returning to our earlier example, oral HIV tests give positive or negative results depending on levels of HIV antibodies detected in saliva.
...
- Tomorrow: Lab 1: Data visualization & data wrangling
