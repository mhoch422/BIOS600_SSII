...
Every homework assignment and lab in this class will be written in Quarto, with a template provided for you to use for the first few assignments. This homework's template can be found on the Canvas site as "hw-1-template.qmd". Download this file and save it to your project with the name hw-01.qmd.

You should now see the new file `hw-01.qmd` under the Files tab in the bottom-right hand side of your RStudio window. Click on `hw-01.qmd` in this window in order to open it up – it is the template for use in this lab!

First, put your name in the space at the top where it says "YOUR NAME GOES HERE".

Fill in answers in the spaces provided: text narrative should be typed directly in the document and any included R code should be typed inside “chunks,” or sections defined by three backticks (the little mark on the same key as the tilde). See the template for more instructions, or ask me.
