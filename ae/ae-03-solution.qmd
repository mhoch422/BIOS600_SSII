...
First filter to only two groups

```{r}
bp_data_plac_mind <- bp_data |>
  filter(group=="Placebo" | group=="Mindfulness")

bp_data_plac_tea <- bp_data |>
  filter(group=="Placebo" | group=="HerbalTea")

bp_data_mind_tea <- bp_data |>
  filter(group=="HerbalTea" | group=="Mindfulness")
```

Next two sample t-test

```{r}
t.test(reduction ~ group,
       data = bp_data_plac_mind,
       mu = 0,
