...
2. To begin, start a new Quarto file. Change the title to Lab 3. Keep the output as html. 

3. "Save As" into your class project folder with a new name: "`lab-03.qmd`".
...
- **Is higher fuel efficiency associated with lower horsepower?**

- We anticipate
  - Lighter cars have higher fuel efficiency
  - Lighter cars have lower horsepower
  
## Partial correlation

  - Car weight could be a **confounder** 
  - Car weight is associated with the exposure (horsepower)
  - Car weight is associated with the outcome (fuel efficiency)

- If I look at the association between fuel efficiency and horsepower I might get a biased estimate
...
- Partial correlation removes weight's influence from both variables, then asks what's left
...
We can also use Spearman's correlation as the method for computing partial correlation:

```{r}

pcor.test(mtcars$mpg, mtcars$hp, mtcars$wt, method = "spearman")
